...
function AlertsPage({ data, open, reload }: { data: DataState; open: (event: DrowsinessEvent) => void; reload: () => Promise<void> }) {
  ...
  async function download() { const response = await fetch(api.exportReportUrl(), { headers: { Authorization: `Bearer ${sessionStorage.getItem("driverguard_token")}` } }); if (!response.ok) return; const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "drowsiness-events.csv"; anchor.click(); URL.revokeObjectURL(url); }
...
function EventDetail({ event, close, saved }: { event: DrowsinessEvent; close: () => void; saved: () => Promise<void> }) {
  ...
  return <Modal title="Chi tiết cảnh báo" close={close} wide><div className="event-detail">{event.imageUrl ? <img src={event.imageUrl} alt="Ảnh minh chứng cảnh báo" /> : <div className="evidence-empty"><Camera />Không có ảnh minh chứng</div>}<div className="event-metrics"><div><small>Loại</small><strong>{event.eventType}</strong></div><div><small>EAR</small><strong>{event.ear ?? "—"}</strong></div><div><small>Confidence</small><strong>{event.confidence == null ? "—" : `${Math.round(event.confidence * 100)}%`}</strong></div><div><small>Nhắm mắt</small><strong>{event.closedDurationMs ? `${event.closedDurationMs} ms` : "—"}</strong></div></div><label>Trạng thái<select value={status} onChange={(e) => setStatus(e.target.value as DrowsinessEvent["status"])}><option value="NEW">Mới</option><option value="ACKNOWLEDGED">Đã xác nhận</option><option value="RESOLVED">Đã xử lý</option></select></label><label>Ghi chú<textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} /></label>{error && <div className="form-error">{error}</div>}<button className="primary-button" onClick={submit} disabled={busy}>{busy ? "Đang lưu..." : "Lưu xử lý"}</button></div></Modal>;
}

function Modal({ title, close, children, wide = false }: { title: string; close: () => void; children: React.ReactNode; wide?: boolean }) { return <div className="modal-backdrop" onMouseDown={close}><section className={`modal ${wide ? "modal-wide" : ""}`} onMouseDown={(e) => e.stopPropagation()}><header><h2>{title}</h2><button onClick={close}><X /></button></header>{children}</section></div>; }
