import { LocationPanel, eventLabel, hasCoordinates } from "./LocationPanel";
...
function Dashboard({ data, openAlert, openSession }: { data: DataState; openAlert: (event: RecentAlert) => void; openSession: (id: string) => void }) {
...
function SessionsPage({ items, users, devices, vehicles, openSession }: { items: MonitoringSession[]; users: User[]; devices: Device[]; vehicles: Vehicle[]; openSession: (id: string) => void }) {
  ...
  return <div className="page-stack"><Toolbar title="Phiên giám sát" description="Theo dõi các phiên được mở từ ứng dụng hoặc thiết bị." search={search} setSearch={setSearch} /><div className="panel table-panel"><table><thead><tr><th>Tài xế</th><th>Thiết bị</th><th>Phương tiện</th><th>Bắt đầu</th><th>Kết thúc</th><th>Trạng thái</th><th>Lịch trình</th></tr></thead><tbody>{filtered.map((s) => <tr key={s.id}><td><strong>{name(s.userId)}</strong></td><td>{device(s.deviceId)}</td><td>{plate(s.vehicleId)}</td><td>{formatDate(s.startedAt)}</td><td>{formatDate(s.endedAt)}</td><td><Badge tone={s.status === "active" ? "green" : "gray"}>{s.status}</Badge></td><td><button className="text-button" onClick={() => openSession(s.id)}>Xem lịch trình</button></td></tr>)}</tbody></table>{!filtered.length && <Empty text="Chưa có phiên giám sát" />}</div></div>;
}
